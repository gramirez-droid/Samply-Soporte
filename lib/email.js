 ...
 *  `adjunto` es opcional: { nombre, url } si el cliente subió un archivo. */
export async function notificarNuevoTicketAAdmins(ticket, clienteNombre, adjunto) {
    ...
    html: emailLayout({
      ...
      bodyHtml: `
        ...
        <div style="margin-top:14px;">
          <div style="color:#6B7A99; font-size:13px; margin-bottom:4px;">Descripción del cliente</div>
          <div style="background:${SAMPLY_BG}; border-radius:8px; padding:10px 14px; font-size:14px;">${ticket.descripcion}</div>
        </div>` : ''}
        ${adjunto?.url ? `
        <div style="margin-top:14px; font-size:14px;">
          <a href="${adjunto.url}" style="color:${SAMPLY_BLUE};">📎 Ver adjunto: ${adjunto.nombre || 'archivo'}</a>
        </div>` : ''}
      `,
