...
import { buscarTicketsValidadosEnNotion, actualizarAssigneeEnNotion } from './notion';
import { notificarCambioEstadoAlCliente } from './email';
...
export async function sincronizarResueltosDesdeNotion() {
  ...
  for (const page of pages) {
    const { rows } = await query(
      `SELECT t.id, t.codigo, t.asunto, t.estado, t.usuario_id, uc.email AS usuario_email
       FROM tickets t
       LEFT JOIN usuarios_cliente uc ON uc.id = t.usuario_id
       WHERE t.notion_page_id = $1`,
      [page.id]
    );
    const ticket = rows[0];
    ...
    await query(
      ...
      [ticket.id, estadoAnterior]
    );

    // Mismo email de "ticket resuelto" que dispara el cambio de estado
    // manual — para el cliente es exactamente lo mismo que se resuelva
    // desde el panel o desde Notion.
    if (ticket.usuario_email) {
      const notif = await notificarCambioEstadoAlCliente(
        { codigo: ticket.codigo, asunto: ticket.asunto },
        ticket.usuario_email,
        'Resuelto',
        estadoAnterior
      ).catch((err) => ({ enviado: false, motivo: err.message }));
      if (!notif.enviado) {
        console.log('[email] No se notificó la resolución vía Notion:', notif.motivo);
      }
    }
...
/**
 * Sync hacia ADELANTE (Samply → Notion): para cada ticket que ya tiene su
 * page creada en Notion, empuja la lista actual de agentes asignados como
 * Assignee — así si asignás/cambiás el agente en Samply DESPUÉS de que se
 * creó la page, y después apretás "Sincronizar con Notion", se refleja allá.
 * No toca nada más de la page (ni Status ni el resto de las properties).
 */
export async function sincronizarAssigneesHaciaNotion() {
  const { rows } = await query(
    `SELECT t.id, t.notion_page_id,
            COALESCE(
     ...
     FROM tickets t
     WHERE t.notion_page_id IS NOT NULL
  ...
  for (const row of rows) {
    try {
      await actualizarAssigneeEnNotion(row.notion_page_id, row.agentes_nombres || []);
      actualizados++;
    } catch (err) {
      console.log(`[notion] No se pudo actualizar Assignee del ticket ${row.id}:`, err.message);
    }
  }
  return actualizados;
}
