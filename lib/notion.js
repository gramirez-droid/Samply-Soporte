 ...
 * `adjuntos` es la lista de adjuntos que ya tiene el ticket (puede venir vacía).
 * Devuelve el notion_page_id, o null si Notion no está configurado.
 */
export async function crearTicketEnNotion(ticket, clienteNombre, agentesNombres = [], adjuntos = []) {
  ...
  if (ticket.descripcion) {
    children.push({
      object: 'block',
      type: 'paragraph',
      paragraph: { rich_text: [{ text: { content: `Descripción del cliente: ${ticket.descripcion}` } }] },
    });
  }
  ...
  for (const adj of adjuntos) {
    children.push({
      object: 'block',
      type: 'paragraph',
      paragraph: { rich_text: [{ text: { content: `📎 ${adj.nombre}`, link: { url: adj.url } } }] },
    });
  }
  ...
  children.push({
    object: 'block',
    type: 'paragraph',
    paragraph: { rich_text: [{ text: { content: 'Ver ticket en Samply', link: { url: urlTicket } } }] },
  });
